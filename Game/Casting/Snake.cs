...
namespace Cycle.Game.Casting
{
    ...
    public class Snake : Actor
    {
        ...
        public void GrowTail(int numberOfSegments, string player)
        {
        ...
        private void PrepareBody(string player)
        {
            ...
            for (int i = 0; i < Constants.SNAKE_LENGTH; i++)
            {
                ...
                string text = i == 0 ? "8" : "#";
                Color color;
                // Sets the colors for the two players
                if (player == "player1")
                {
                    color = i == 0 ? Constants.RED : Constants.RED;
                    position = new Point(x - i * Constants.CELL_SIZE, y / 2);
                }
                else 
                {
                    color = i == 0 ? Constants.GREEN : Constants.GREEN;
                    position = new Point(x - i * Constants.CELL_SIZE, y / 5);
                }
